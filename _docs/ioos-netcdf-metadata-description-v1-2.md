...
| 1.2 | Updated to reflect new IOOS attribution guidance and ERDDAP implementation: <br>* Add `info_url` <br>* Add `institution_url` <br>* Make `publisher_url` required <br>* Clarify vocabulary for `contributor_role`<br>* Clarify use of `contributor_name` and `contributor_role` for multiple contributors <br>* Clarify use of `platform` variable <br>* Add `ioos_exclude` flag | 2018-xx-xx |
...
:--------- | :-------: | :------------------- | :--------: | :-------:
contributor_name | ACDD | The name of any individuals or institutions that contributed to the creation of this data. Combined with the **`contributor_role`**, it provides the full description of the contributor. Multiple names should be given in CSV format. | global | required
contributor_role | ACDD | The role of any individuals or institutions that contributed to the creation of this data. The [NERC CI_RoleCode vocabulary](http://vocab.nerc.ac.uk/collection/G04/current/) should be used. Multiple roles should be given in CSV format, and presented in the same order and number as the names in `contributor_names`.<br>For the IOOS ncSOS, **`contributor_role = "sponsor"`** defines a person, group, or organization’s full or partial support of an IOOS activity, asset, model, or product. | global | required
...
institution_url  | IOOS | URL of the person or group that collected the data. | global | required
ioos_exclude  | IOOS | If set to true, then *exclude* this dataset from IOOS systems such as the catalog and sensor map. | global | not required
...
platform | ACDD/NCEI |  **When used as a global attribute:** Name of the *type* of platform(s) that supported the sensor data used to create this data set or product. Platforms can be of any type, including satellite, ship, station, aircraft or other. The controlled vocabulary must be indicated in the `platform_vocabulary` field. <br><br>Example (global variable): {::nomarkdown}<i><ul> <li> <b><code>platform "buoy";</code></b> <li><b><code>platform_vocabulary "IOOS Platform Vocabulary";</code></b></i> </ul>{:/} <br><br> **When used as a variable attribute:** This attribute can be used with a geophysical variable to identify the platform that was used in the collection of the data. The value of the attribute should be set to another variable which contains the details of the platform. <br><br>For an example, see the [NCEI Templates timeSeries example (Stearns Wharf)](https://data.nodc.noaa.gov/thredds/catalog/ioos/sccoos/stearns_wharf/catalog.html?dataset=ioos/sccoos/stearns_wharf/stearns_wharf-2013.nc). | global or variable | required
